...
function interface_load() {
	...
	settings.speed = Number(elements_settings["controls_images_settings_speed"].value);

	// divide the image count of each source to the total number of sources
	var count = Math.floor(settings.count / settings.sites.length);

	// load every selected plugin
	for(var site in settings.sites) {
		plugins_load(settings.sites[site], settings.keywords, count);
	}
}
...
function interface_update_media_controls_label() {
	...
	var str =
		"Images loaded: " + total_images + "<br/>" +
		"Estimated duration: " + date_string;
	label.innerHTML = str;
}
...
function interface_init() {
		...
		controls.appendChild(controls_images);
		{
			...
			controls_images.appendChild(controls_images_settings);
			{
				...
				controls_images_settings.appendChild(controls_images_settings_count);
				{
					...
					controls_images_settings_count_input.setAttribute("step", "10");
					controls_images_settings_count_input.setAttribute("min", "10");
				...
				// interface HTML: controls, images, settings, speed
				var controls_images_settings_speed = document.createElement("p");
				controls_images_settings_speed.innerHTML = "Speed:<br/>";
				controls_images_settings.appendChild(controls_images_settings_speed);
				{
					// interface HTML: controls, images, settings, speed, input
					var controls_images_settings_speed_input = document.createElement("input");
					controls_images_settings_speed_input.setAttribute("id", "controls_images_settings_speed");
					controls_images_settings_speed_input.setAttribute("type", "number");
					controls_images_settings_speed_input.setAttribute("value", "10");
					controls_images_settings_speed_input.setAttribute("step", "5");
					controls_images_settings_speed_input.setAttribute("min", "5");
					controls_images_settings_speed_input.setAttribute("max", "100");
					controls_images_settings_speed.appendChild(controls_images_settings_speed_input);
				}
	...
	document.body.appendChild(media);
	{
		...
		media.appendChild(media_controls);
		{
			...
			media_controls_play.setAttribute("onclick", "interface_play_stop()");
			...
			media_controls_label.setAttribute("style", "position: absolute; top: 50%; left: 0%; width: 100%; height: 50%");
